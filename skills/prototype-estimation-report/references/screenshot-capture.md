...
## 5. Make the thumbnails clickable to enlarge

A 380px-wide thumbnail is too small to actually read on a screen the reviewer wants to look closely at, and `report-template.html` ships a click-to-enlarge lightbox for exactly this reason — but the lightbox is only as good as the image behind it. Run `extract_screenshots.py` **twice** against the same session transcript: once at the small thumbnail size for the inline card (`--resize-width 380 --quality 55`, written to `<out>/processed/`), and once more at a genuinely larger size for the enlarged view (`--resize-width 900 --quality 68`, written to a separate `--out` directory, e.g. `<scratch>/zoom`). Embed the small one as the `<img src>` and the larger one as that same `<img>`'s `data-zoom` attribute — the template's lightbox script reads `data-zoom` first and falls back to `src` if it's missing. Never point `data-zoom` at the same 380px file as `src`; enlarging a thumbnail via CSS alone just blows up the same blurry pixels, it doesn't reveal more detail. Check the combined total of both sets stays comfortably under the ~1MB danger zone from step 4 above (a dozen screens at 900px/quality 68 typically lands around 300–450KB on top of the thumbnails) — if a report has an unusually large number of screens, drop the zoom quality or width before dropping the thumbnails back to full-res.

## 6. If the Browser pane stops responding
