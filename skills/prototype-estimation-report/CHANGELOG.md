...
## v1.4.0 — reorganized so a fresh agent (or teammate) can actually follow the logic, no behavior change (2026-09-14)

Three fast rounds (v1.1.0–v1.3.0) left `estimation-logic.md` in *patch-discovery order* rather than *reading order* — each new rule got appended wherever there was room, not where a first-time reader would expect it. Two concrete symptoms: a "Never wire a script-triggered print or download button" rule sat nested three levels deep inside "Writing so a non-technical reader understands it," even though it has nothing to do with wording; and Functions' rules existed in two separate places (a "Readiness Detail and Functions" subsection partway through the file, and a standalone "## Functions readiness" section at the very end) that said overlapping things in different words.

Reorganized `estimation-logic.md` into the order you actually need it in — scope, then how to write for the report's real audience, then how to judge and word Acceptance Criteria/Readiness/Components/Functions (all four together now, since they're judged and written as one pass per story), then risk severity, then a proper "Environment gotchas" section for the print-button rule and a one-line pointer to `screenshot-capture.md`'s own Browser-pane gotcha instead of leaving it orphaned. Merged the two Functions locations into one. No rule content changed — every sentence that existed before still exists, just filed under a heading that matches where you'd look for it.

Also trimmed `SKILL.md` Process step 4 from a single flat list of nine unrelated bullets into four labeled groups (Structure / User Story & Acceptance Criteria / Readiness–Components–Functions / Language & environment), each pointing at the matching `estimation-logic.md` section by name instead of re-explaining the reasoning inline — one source of truth for *why*, so a future patch only has to land in one file instead of two risking drift between them (which is exactly what produced the mismatched rule locations this version fixes).
